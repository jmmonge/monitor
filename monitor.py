    ...
    with open(VEEAM_JSON_FILE, "r", encoding="utf-8-sig") as f:
        try:
            ...
            for j in v_data:
                ...
                last_run_str = j.get('LastRun')
                f_v = formatear_fecha_veeam(last_run_str)
                
                # Cálculo de días para el color de la fecha
                try:
                    f_dt = datetime.strptime(last_run_str, "%Y-%m-%d %H:%M:%S")
                    dias = (now - f_dt).days
                    c_fecha = "badge-green" if dias == 0 else "badge-orange" if dias <= 2 else "badge-red"
                except:
                    c_fecha = "badge-yellow"  # Si no se puede parsear la fecha, usar amarillo como advertencia

                cls_res = "badge-green" if res == "Success" else "badge-red"
                fecha_col = f'<span class="badge {c_fecha}">{f_v}</span>'
...
for t in tareas_hbs3:
    ...
    if inf:
        # 1. Procesar la Fecha (mantenemos tu criterio original)
        ...
        if dias == 0:
            ...
            c_fecha = "badge-orange"
        else:
            c_fecha = "badge-red"
        
        f_str = f'<span class="badge {c_fecha}">{f_dt.strftime("%d/%m/%Y %H:%M")}</span>'

        # 2. Procesar el Estado (OK -> verde, ERROR -> rojo)
        estado_valor = inf.get("estado", "ERROR") # Por defecto ERROR si no existe la clave
